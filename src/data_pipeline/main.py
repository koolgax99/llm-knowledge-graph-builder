...
def main(user_query=None, min_year=None, max_year=None, experiment_name=None, output_folder=None):
    ...
    if metadata_id:
        try:
            ...
            pmid_csv_filename = os.path.join(output_folder or "data", f"{experiment_name}_pubmed_results.csv")
    ...
    try:
        ...
        pdf_download_folder = os.path.join(output_folder or "data", f"{experiment_name}_pubmed_downloads")
        os.makedirs(pdf_download_folder, exist_ok=True)
        errors_csv_filename = os.path.join(output_folder or "data", f"{experiment_name}_pubmed_download_errors.csv")
        run_pubmed_downloader(pdf_download_folder, pmid_csv_filename, errors_csv_filename)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment_name", type=str, default="pubmed_search", help="Name of the experiment for output files")
    parser.add_argument("--output_folder", type=str, default="/home/exouser/masters-thesis/ai-knowledge-graph-main/test_data", help="Folder to save output files")
