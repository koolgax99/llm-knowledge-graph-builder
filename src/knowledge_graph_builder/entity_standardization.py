...
def standardize_entities(triples, config):
    ...
    for group_key, variants in entity_groups.items():
        if len(variants) == 1:
            ...
            standard_form = sorted(
                variants, key=lambda x: (-variant_counts[x], len(x))
            )[0]
            for variant in variants:
    ...
    for i, entity1 in enumerate(sorted_standards):
        ...
        for entity2 in sorted_standards[i + 1 :]:
            ...
            if e1_words.issubset(e2_words) and len(e1_words) > 0:
                ...
                if (
                    shared_stems
                    and (len(shared_stems) / max(len(stems1), len(stems2))) > 0.5
                ):
    ...
    if len(filtered_triples) < len(standardized_triples):
        print(
            f"Removed {len(standardized_triples) - len(filtered_triples)} self-referencing triples"
        )

    print(
        f"Standardized {len(all_entities)} entities into {len(set(standardized_entities.values()))} standard forms"
    )
...
def infer_relationships(triples, config):
    ...
    lexical_triples = _infer_relationships_by_lexical_similarity(
        all_entities, valid_triples
    )
    if lexical_triples:
    ...
    if len(filtered_triples) < len(unique_triples):
        print(
            f"Removed {len(unique_triples) - len(filtered_triples)} self-referencing triples"
        )
...
def _infer_relationships_with_llm(triples, communities, config):
    ...
    for i, comm1 in enumerate(large_communities):
        for j, comm2 in enumerate(large_communities):
            ...
            user_prompt = get_relationship_inference_user_prompt(
                entities1, entities2, triples_text
            )
            ...
            try:
                ...
                if inferred_triples and isinstance(inferred_triples, list):
                    ...
                    print(
                        f"Inferred {len(new_triples)} new relationships between communities"
                    )
                else:
                    print(
                        "Could not extract valid inferred relationships from LLM response"
                    )
...
def _infer_within_community_relationships(triples, communities, config):
    ...
    for community in sorted(communities, key=len, reverse=True)[:3]:
        ...
        connections = {
            (a, b): False
            for a in community_entities
            for b in community_entities
            if a != b
        }
        ...
        user_prompt = get_within_community_inference_user_prompt(
            pairs_text, triples_text
        )
        ...
        try:
            ...
            if inferred_triples and isinstance(inferred_triples, list):
                ...
                print(
                    f"Inferred {len(inferred_triples)} new relationships within communities"
                )
            else:
                print(
                    "Could not extract valid inferred relationships from LLM response"
...
def _infer_relationships_by_lexical_similarity(entities, triples):
    ...
    for i, entity1 in enumerate(entities_list):
        for entity2 in entities_list[i + 1 :]:
            ...
            if (entity1, entity2) in existing_relationships or (
                entity2,
                entity1,
            ...
            if (entity1, entity2) in processed_pairs or (
                entity2,
                entity1,
            ) in processed_pairs:
                continue
            ...
            if shared_words:
                ...
                if len(main_shared) >= 4:  # Only consider significant shared words
                    if e1_lower.startswith(main_shared) and not e2_lower.startswith(
                        main_shared
                    ):
                        new_triples.append(
                            {
                    ...
                    elif e2_lower.startswith(main_shared) and not e1_lower.startswith(
                        main_shared
                    ):
                        new_triples.append(
                            {
