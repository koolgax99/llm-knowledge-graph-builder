...
def call_openai(
    ...
    client = OpenAI(
        base_url="https://llm.jetstream-cloud.org/llama-4-scout/v1", api_key="empty"
    )
    ...
    try:
        ...
        if complete_json:
            try:
                ...
                fixed_json = re.sub(
                    r'(\s+)\w+\s+"([^"]+)"(\s*):(\s*)', r'\1"\2"\3:\4', fixed_json
                )
                # Fix stray words after quoted keys
                fixed_json = re.sub(
                    r'(\s+)"([^"]+)"\s+\w+(\s*):(\s*)', r'\1"\2"\3:\4', fixed_json
                )
                # Fix trailing commas
            ...
            if objects:
                ...
                try:
                    ...
                    # Try to fix missing quotes around keys
                    fixed_json = re.sub(
                        r"(\s*)(\w+)(\s*):(\s*)", r'\1"\2"\3:\4', reconstructed_json
                    )
                    # Fix trailing commas
                    fixed_json = re.sub(r",(\s*[\]}])", r"\1", fixed_json)
