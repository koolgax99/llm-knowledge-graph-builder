...
def _load_html_template():
    ...
    template_path = os.path.join(
        os.path.dirname(__file__), "templates", "graph_template.html"
    )
    try:
        ...
        return (
            '<div id="mynetwork" class="card-body"></div>'  # Fallback to basic template
        )


def visualize_knowledge_graph(
    triples, output_file="knowledge_graph.html", edge_smooth=None, config=None
):
    """
    ...
    for triple in triples:
        ...
        G.add_edge(
            ...
            color="#555555"
            if is_inferred
            else None,  # Lighter color for inferred relationships
        )
...
def _detect_communities(G_undirected, all_nodes):
    ...
    try:
        ...
        for node in all_nodes:
            ...
            community_id = (
                max(0, node_degree) % 8
            )  # Using modulo 8 to limit number of colors
...
def _calculate_node_sizes(all_nodes, betweenness, degree, eigenvector):
    ...
    for node in all_nodes:
        ...
        betweenness_norm = (
            betweenness.get(node, 0) / max_betweenness if max_betweenness > 0 else 0
        )
        eigenvector_norm = (
            eigenvector.get(node, 0) / max_eigenvector if max_eigenvector > 0 else 0
        )
...
def _save_and_modify_html(net, output_file, community_count, all_nodes, triples):
    ...
    html = html.replace(
        '<div id="mynetwork" class="card-body"></div>', _load_html_template()
    )
    ...
    html = html.replace(
        "<h1></h1>",
        f"<h1>Knowledge Graph - {len(all_nodes)} Nodes, {len(triples)} Relationships, {community_count} Communities</h1>",
    )
...
def sample_data_visualization(
    output_file="sample_knowledge_graph.html", edge_smooth=None, config=None
):
    """
    ...
    sample_triples = [
        ...
        },
        {
            "subject": "Industrial Revolution",
            "predicate": "led to",
            "object": "rise of capitalism",
        },
        {
            "subject": "Industrial Revolution",
            "predicate": "led to",
            "object": "new labor movements",
        },
        {
            "subject": "Industrial Revolution",
            "predicate": "fueled by",
            "object": "technological innovations",
        },
    ...
    stats = visualize_knowledge_graph(
        sample_triples, output_file, edge_smooth=edge_smooth, config=config
    )
...
if __name__ == "__main__":
    ...
    print(
        f"- sample_knowledge_graph_config.html: Using {config_description} from config.toml"
    )
